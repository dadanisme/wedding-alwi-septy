...
- **Seksi Pembuka** (`components/sections/pembuka.tsx`, dirender di `/` setelah Ayat) — salam "Bismillahirrahmanirrahim" + "Assalamu'alaikum..." + kalimat pengantar, di atas foto adat Sunda gelap dengan scrim, wave divider atas-bawah (transisi dari/ke seksi krem), dan sulur pojok bawah. Konten baru di `lib/event-config.ts` (`openingGreeting`) — tidak ada di PRD (PRD §4.2 cuma bilang "Salam dan kalimat pengantar"), ditranskrip dari mockup yang sama dengan Ayat. **Beda dari Sampul:** ponsel & desktop pakai foto berbeda (`openingPhotos.portrait`/`landscape`, adat-sunda-03/04) alih-alih satu foto di-crop ulang, karena mockup minta foto lanskap PENUH untuk desktop dan beberapa foto klien memang berorientasi lanskap asli — lihat `docs/DECISIONS.md`. Dua utility tipografi baru (`text-pembuka-greeting(-lg)`, `text-pembuka(-lg)`) dan 2 utility scrim baru (`bg-pembuka-scrim(-lg)`) ditranskrip dari mockup. `/code-review` menemukan kontainer bertinggi tetap + `overflow-hidden` berisiko memotong teks diam-diam kalau salam direvisi lebih panjang — diperbaiki jadi `min-h`. Fix pertama (taruh `h-full` + `justify-end` di div pembungkus teks) ternyata regresi: `h-full` tidak resolve terhadap parent `min-height` (bukan `height` eksplisit), jadi teks salam malah nongol di paling atas seksi — **lolos ke commit pertama karena verifikasi visual sesi itu dilakukan sebelum fix ini, dan tidak diulang setelahnya.** Ketahuan dari screenshot manusia di device asli, lalu diperbaiki dengan memindah `justify-end` ke `<section>` itu sendiri (lihat `docs/DECISIONS.md`). Diverifikasi ulang (rect DOM + screenshot) di ~500×844 (devtools MCP tidak bisa di-resize ke 390 persis, floor-nya 500px, tapi tidak ada breakpoint CSS di antara 390–1024 jadi tidak berpengaruh) & 1280×900. Manusia lalu melaporkan bug lain dari monitor lebar (>1920px): kepala pasangan terpotong di foto latar karena kontainer jadi sangat pipih (tinggi tetap, lebar 100vw tak terbatas) sementara `object-cover` defaultnya crop dari tengah. Diperbaiki jadi `object-top` (lihat `docs/DECISIONS.md`), diverifikasi ulang di 390px/1280px/2200px — kepala aman di ketiganya, 1280px (breakpoint referensi mockup) masih cocok nuansanya. **Belum dikonfirmasi manusia di ponsel asli** untuk versi final ini.
