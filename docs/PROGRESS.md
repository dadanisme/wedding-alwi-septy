...
- **Ornamen Botani Foil Emas untuk Pembatas Wave Sampul** (`components/sections/sampul.tsx`, `public/ornaments/gold-floral-spray.webp`) — motif bunga & sulur botani foil emas di-generate via AI langsung dari acuan visual referensi klien, dikonversi ke WebP transparan (alpha channel), dan dipasang di atas garis lengkung wave Sampul (kiri & kanan). Ukuran di mobile diperbesar (`w-[128px]` kanan, `w-[110px]` kiri) dengan orientasi mekar mengarah ke dalam foto. Aset flourish pendukung lainnya disiapkan di `public/ornaments/`. Disetujui pengguna.
