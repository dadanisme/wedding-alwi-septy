...
- **Fitur Interaksi "Buka Undangan" & Pemutar Musik Latar** (`components/invitation-experience.tsx`, `components/music-player.tsx`, `components/sections/sampul.tsx`, `components/sections/ayat.tsx`, `lib/event-config.ts`, `public/audio/sabilulungan.mp3`) — Sampul mengunci scroll awal (`h-dvh overflow-hidden` pada `<main>`, `overflow: hidden` pada `html`/`body`, serta pencegahan drag swipe) sesuai PRD §4.2. Tombol "Buka Undangan" membuka kunci scroll, memulai pemutaran audio latar (Sabilulungan - Sundanese Gamelan, `preload="none"` sesuai PRD §7.1), dan melakukan *smooth-scroll* sinematik berbasis kurva *spring-like ease-in-out cubic* sepanjang 1,5 detik (1500ms) ke seksi Ayat (`#ayat`), menggantikan scroll instan bawaan browser. Tombol kontrol mengambang (floating music button) muncul di kanan bawah dengan ikon speaker universal (gelombang suara saat aktif / garis silang saat jeda), mendukung ponsel (390px), desktop (1280px), dan terkunci rapi di dalam container `max-w-page` pada ultrawide (>1440px). Diverifikasi via Chrome DevTools MCP.

## Berikutnya

- Seksi Buku Tamu (Seksi 9)
- Seksi Hadiah (Seksi 10)
- Seksi Penutup (Seksi 11)
