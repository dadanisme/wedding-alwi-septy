...
## Seksi Galeri: Penambahan 2 foto baru, penghapusan 2 duplikat/1 run-ganjil, dan perbaikan tinggi baris grid

**Konteks:** user memberi 2 foto prewedding baru (`Document Name SE_3.jpg`, `Document Name SE_9.jpg`, ditemukan di Downloads) untuk ditambahkan ke galeri.

**Keputusan konten:**
1. 2 foto baru diproses dengan pipeline identik intake 21 foto awal (`sips --resampleHeightWidthMax 2000` + kompresi JPEG q78) dan disimpan sebagai `modern-13.jpg`/`modern-14.jpg` (busana modern, taman bunga ungu — sama kategori dengan set `modern-*` yang sudah ada).
2. `modern-09.jpg` dihapus dari `galleryPhotos` — user mengidentifikasi ini duplikat/jepretan beruntun dari `modern-08.jpg` (pose, framing, dan lokasi identik), diverifikasi visual sebelum dihapus. Sama persis dengan kasus `modern-06.jpg` di sesi sebelumnya.
3. `adat-sunda-03.jpg` dihapus dari `galleryPhotos`. Berbeda dari 2 keputusan di atas, ini BUKAN soal duplikat/kualitas foto — foto ini legitimate, tapi posisinya (`adat-sunda-01/02/03`, rangkaian 3 foto potret berturut-turut) memicu heuristik run-ganjil `buildGalleryTiles()` yang memaksanya jadi tile lebar (`col-span-2`, dipotong sangat berbeda dari 2 foto di sebelahnya) SEKALIGUS menyisakan sisa 2 kolom kosong di baris terakhir grid desktop (total unit span grid berubah dari 28 — kelipatan 4, pas — jadi 30 — sisa 2 — akibat net +1 foto dari poin 1 & 2). Dibuktikan lewat simulasi matematis (bukan tebakan) bahwa menghapus SALAH SATU dari `adat-sunda-01/02/03` mengembalikan total ke 28 dan menyelesaikan KEDUA masalah sekaligus. Dipilih `adat-sunda-03` spesifik karena itu foto yang sedang bermasalah (tidak perlu reorder foto lain), dan mempertahankan `adat-sunda-02` (juga dipakai sebagai foto sampul desktop) serta `adat-sunda-01` (variasi candid) di galeri.
4. **Ditolak:** menambah foto lagi (bukan menghapus) untuk memperbaiki isu run-ganjil. 3 foto lanskap yang sebelumnya dihapus dari galeri (`modern-10.jpg`, `modern-12.jpg`, `adat-sunda-05.jpg`, semua masih ada di `public/photos/`) dicek ulang sebagai kandidat — ternyata semuanya kurang cocok secara kualitas/nada (`modern-10`: ekspresi kurang pas; `modern-12`: efek gelembung digital yang kontras dengan nada elegan galeri; `adat-sunda-05`: foto prop novelty memegang mock-up majalah, beda nada dari foto lain). Lagipula ketiganya berorientasi LANSKAP (bukan potret seperti dugaan awal — sempat salah asumsi `adat-sunda-05` potret sebelum dicek `sips`), jadi menambahnya TIDAK memperbaiki rangkaian ganjil `01/02/03` sama sekali (foto lanskap tidak bisa memperpanjang rangkaian potret) — hanya kebetulan bisa memperbaiki total unit span kalau ditambahkan di tempat lain, itu pun harus dikombinasikan dengan perbaikan terpisah untuk isu run-ganjilnya.

**Bug baru ditemukan (bukan disebabkan penghapusan `adat-sunda-03`, sudah ada sebelum sesi ini juga) — tinggi baris grid tidak rata:**

Setelah `adat-sunda-03` dihapus, baris grid desktop yang sebelumnya berisi `adat-sunda-01/02/03` (3 tile potret) berubah jadi `adat-sunda-01/02` (2 tile potret) + `adat-sunda-06` (1 tile lanskap GENUINE, bukan hasil promosi) — kombinasi yang sebelumnya tidak pernah diuji. User melaporkan lewat 2 screenshot berturut-turut bahwa tinggi tile lanskap itu tidak sama dengan tile potret di sebelahnya. Investigasi (simulasi + verifikasi live `getBoundingClientRect`/`getComputedStyle` di browser, bukan tebakan) menemukan kombinasi identik SUDAH ADA sebelum sesi ini juga (baris `adat-sunda-09` + `01` + `02`, sebelum `modern-13/14` ditambahkan) — cuma belum pernah ketahuan karena belum ada yang screenshot baris itu dari dekat.

Perbaikan berjalan 3 iterasi sebelum benar-benar presisi piksel:
1. **Percobaan 1 (aspect-[4/3] tetap):** memakai rasio yang sama dengan tile potret yang dipromosikan (sudah ada sejak sesi sebelumnya untuk kasus lain). Mengurangi selisih dari ~48px jadi ~7-8px, TAPI tidak nol — lebar sel span-2 sebenarnya `2×lebar-kolom + gap` (bukan persis `2×lebar-kolom`), jadi rasio tetap manapun cuma bisa mendekati, tidak pernah pas persis, dan selisihnya membesar di layar lebih lebar (linear terhadap lebar kolom, gap tetap dalam piksel). User melaporkan lagi: "masih ga sama".
2. **Percobaan 2 (`aspect-auto` + `h-full`, stretch mengikuti CSS Grid):** melepas rasio tetap sepenuhnya dan biarkan tile mengikuti tinggi baris (`align-items: stretch` bawaan grid item tanpa ukuran intrinsik). Secara teori exact by construction — tapi PENGUKURAN LIVE (bukan cuma baca kode) menunjukkan masih ada selisih seragam ~7px di SEMUA baris (potret maupun lanskap, campuran maupun tidak) — jadi bukan lagi soal kombinasi span, tapi sesuatu yang lebih dasar.
3. **Akar masalah sesungguhnya, ditemukan lewat pengujian langsung `display:none` pada tile lanskap (membuktikan selisih tetap ada bahkan tanpa tile itu) dan `getComputedStyle` pada elemen `<button>`:** `<button>` HTML defaultnya `display: inline-block`. Elemen inline-block di dalam parent block biasa (`ScrollReveal`) selalu menyisakan sedikit ruang tak terlihat di bawahnya untuk descender font pada baris-teks tersembunyi yang otomatis dibentuk browser — efek klasik yang SERAGAM di semua tile (makanya baru ketahuan sekarang: tile yang seragam rasionya kehilangan tinggi yang sama, jadi tidak pernah kelihatan beda). **Perbaikan final:** tambah `block` ke className tombol (`components/sections/galeri.tsx`), menggantikan `inline-block` bawaan. Diverifikasi live: SEMUA baris kelebihan ~7px identik sebelum ini, nol sesudahnya — `getComputedStyle().height` tile potret dan tile lanskap yang di-stretch jadi PERSIS SAMA (461.25px di 1280px, 521.25px di 2200px, bukan mendekati).

**Perubahan kode (`components/sections/galeri.tsx`):**
1. `simulateRows()` diekstrak jadi helper terpisah, dipakai ulang oleh `assertNoGridGaps`, `matchRowHeightsAtLg`, dan `assertRowHeightsMatch` (sebelumnya cuma dipakai `assertNoGridGaps`).
2. `matchRowHeightsAtLg()` (baru): mendeteksi baris 4-kolom yang mencampur tile span-1 dan span-2, menandai SEMUA tile span-2 di baris itu (`tile.stretchAtLg = true`) — baik tile lanskap genuine maupun tile potret yang dipromosikan (unifikasi, bukan 2 mekanisme berbeda untuk masalah yang sama).
3. `assertRowHeightsMatch()` (baru, dipanggil untuk 2 dan 4 kolom, mirror pola `assertNoGridGaps`): menghitung tinggi relatif tiap tile dari `aspect`-nya sendiri (tile `stretchAtLg` dilewati saat memeriksa breakpoint `lg` karena dijamin cocok oleh CSS, bukan oleh kesamaan rasio) dan `throw` kalau ada baris dengan tinggi tidak sama — mem-fail-kan build kalau kombinasi baru nanti (galeri diedit lagi) memunculkan gap visual serupa yang tidak tertangani `matchRowHeightsAtLg`. Diverifikasi bekerja dengan sengaja men-disable fix sementara (`matchRowHeightsAtLg` dikomentari) — build gagal keras dengan pesan yang tepat menyebut baris yang bermasalah, bukan diam-diam lolos.
4. `<button>` diberi `block` (bukan `inline-block` bawaan browser).
5. Class Tailwind untuk override `lg` DITULIS SEBAGAI LITERAL PENUH lewat ternary (`photo.stretchAtLg ? "lg:aspect-auto lg:h-full" : ""`), BUKAN dirakit dari potongan string (`` `lg:${...}` ``) — Tailwind men-scan teks sumber untuk nama class utuh, string yang dirakit saat runtime dari potongan tidak terdeteksi scanner-nya sehingga CSS-nya tidak pernah ter-generate meski logika JS-nya benar. Sempat jadi bug tersendiri di percobaan 1 sebelum ketahuan dan diperbaiki.

**Verifikasi akhir:** `assertNoGridGaps` × 2 dan `assertRowHeightsMatch` × 2 baru adalah efek samping saat modul `galeri.tsx` dimuat, BUKAN bagian dari `tests/guest-management.test.ts` (satu-satunya berkas `bun test`, tidak pernah meng-import `galeri.tsx`/`galleryPhotos`) — jadi yang benar-benar mengeksekusi keempatnya adalah `bun run build` (prerender statis rute `/`), bukan `bun test`. Ini juga diverifikasi sengaja: `matchRowHeightsAtLg` dikomentari sementara lalu `bun run build` dijalankan ulang, dan build gagal keras persis di baris `assertRowHeightsMatch` dengan pesan yang menyebut baris bermasalah — baru setelah itu dipastikan `bun run build` benar-benar melindungi regresi ini, bukan diam-diam lolos. `bun test` 15/15 dan `bun run lint` 0 error/warning tetap lolos, tapi keduanya tidak menyentuh logika galeri sama sekali. Verifikasi visual + pengukuran piksel langsung (Chrome DevTools MCP, `getBoundingClientRect`/`getComputedStyle`, bukan cuma screenshot) di 390px (mobile 2-kolom, tile span-2 tetap rasio asli seperti sebelumnya, tidak berubah), 1280px, dan 2200px (ultrawide, dalam `max-w-page`) — tinggi seluruh tile dalam satu baris PERSIS sama di breakpoint `lg`, bukan mendekati. Total foto galeri: 17 (awal sesi) + 2 baru (`modern-13`/`14`) − 1 duplikat (`modern-09`) − 1 (`adat-sunda-03`, perbaikan run-ganjil) = **17 foto**. **Belum dikonfirmasi manusia di ponsel asli.**

---

## Naskah Love Story diganti total — "Roadmap to Our Wedding Day"

**Keputusan:** naskah & struktur seksi Love Story (`lib/event-config.ts` → `loveStoryConfig`, `components/sections/love-story.tsx`) diganti total dari linimasa hubungan 2019-2026 (transkrip mockup awal) menjadi naskah "Roadmap to Our Wedding Day" yang diberikan klien sesi ini — 4 langkah pasca-lamaran menuju hari-H, bukan riwayat kenalan. `sectionLabel` ikut berubah dari "Love Story" ke "Roadmap to Our Wedding Day" (termasuk `aria-label` seksi).

**Alasan:** ini naskah final dari klien, menggantikan konten placeholder transkrip mockup. Item "Naskah final Love Story" di PROGRESS.md sebelumnya berstatus menunggu klien (PRD §11 poin 3) — meski naskah baru ini tidak menyertakan tahun eksplisit seperti yang tadinya diharapkan item tersebut, ini tetap naskah resmi yang diberikan, bukan tebakan.

**Perubahan struktur data (`LoveStoryMoment`):**
- Field `year` diganti jadi `step` ("1"–"4"), lalu badge angka tersebut dihapus lagi sepenuhnya dari tampilan (mobile & desktop) atas permintaan langsung user di sesi yang sama — `step` tetap ada di data model hanya untuk React `key` dan teks placeholder foto (`"1 · 4:3"`, dsb.), tidak lagi dirender sebagai elemen visual.
- Field baru `subtitle` (wajib) ditambahkan — title & subtitle sengaja berbahasa Inggris (gaya klien, dikonfirmasi eksplisit lewat AskUserQuestion), body (`mobileDescription`/`desktopDescription`) tetap Bahasa Indonesia. **Satu-satunya seksi di situs dengan campuran bahasa ini** — pengecualian yang disengaja, bukan lupa terjemahan.
- `mobileDescription`/`desktopDescription` dibuat opsional — item ke-4 ("And Finally We're Almost There") hanya punya title + subtitle (kalimat "After all the planning..." adalah subtitle-nya, dikonfirmasi user), tidak ada body paragraf terpisah seperti item 1-3. Komponen dibuat render kondisional (`{moment.mobileDescription && ...}`) untuk kasus ini.

**Bug typografi ditemukan saat verifikasi visual desktop (1280px), tidak disebabkan perubahan ini tapi mempengaruhi field baru:** pola utility `text-story-*-lg` di `app/globals.css` (dipakai `title-lg`, `desc-lg`, dan sebelumnya `year-lg`) hanya berisi properti DELTA (font-size, letter-spacing) — bukan definisi lengkap — dan komponen desktop memakainya SENDIRIAN tanpa base class (`className="text-story-title-lg text-ink"`, bukan `text-story-title text-story-title-lg`, beda dari pola section lain seperti `ayat.tsx` yang menulis `text-ayat-title lg:text-ayat-title-lg` sekaligus). Dibuktikan lewat `getComputedStyle`: judul desktop yang seharusnya Cormorant Garamond 500 malah jatuh ke Crimson Pro 400 (font body, bukan display) karena `font-family`/`font-weight` tidak pernah ter-set untuk varian desktop. **Bug ini SUDAH ADA sebelum sesi ini** (bukan disebabkan perubahan copy) dan TIDAK diperbaiki di sesi ini — di luar scope "ganti copy", dan menyentuhnya berarti mengubah tampilan title/desc section yang sudah dikonfirmasi manusia di ponsel (meski konfirmasi itu hanya mobile, bukan desktop). **Perlu sesi terpisah untuk audit & perbaiki seluruh pola `-lg` di section ini** (kemungkinan section lain juga terdampak kalau memakai pola serupa tanpa `lg:` prefix + base class).
