...
## Seksi Pembuka: `object-top` untuk foto latar, bukan `object-center` default

**Keputusan:** kedua foto latar Pembuka (`openingPhotos.portrait` & `.landscape`) pakai `object-cover object-top`, bukan `object-cover` polos (yang defaultnya `object-center`).

**Alasan:** ditemukan dari screenshot manusia di monitor lebar (>1920px). Kontainer seksi ini tingginya nyaris tetap (`min-h-[660px]`, dan konten saat ini pas di situ) sementara lebarnya `100vw` tak terbatas — di monitor lebar rasio kontainer jadi jauh lebih pipih dari rasio foto sumber (mis. 2200×660 ≈ 3.3:1 vs foto lanskap ≈ 1.5:1). `object-center` (default) memotong rata dari atas & bawah foto secara proporsional terhadap makin lebarnya jendela crop yang tersisa, dan karena kepala pasangan tidak persis di tengah vertikal foto, potongan itu mengenai kepala duluan di lebar layar yang cukup ekstrem — pas di breakpoint referensi mockup (1280px) ini tidak kelihatan karena rasio kontainer di situ masih dekat dengan rasio foto. `object-top` mengunci crop ke tepi atas foto (area kepala) dan mengorbankan bagian bawah (kaki/lantai) duluan saat jendela makin pipih — jauh lebih aman untuk foto orang berdiri.

**Konsekuensi:** di breakpoint referensi 1280px, framing sedikit berubah (headroom di atas kepala sedikit lebih banyak, badan bagian bawah sedikit lebih terpotong) dibanding `object-center` — diverifikasi ulang secara visual, hasilnya masih cocok dengan nuansa mockup (malah selaras dengan catatan mockup desktop "pasangan diletakkan di sepertiga atas"). Tidak menyelesaikan kasus super-ekstrem (mis. rasio >4:1) — kalau nanti ada laporan serupa di lebar yang lebih ekstrem lagi, pertimbangkan menaikkan `min-h` lewat breakpoint tambahan alih-alih terus menambal lewat object-position.

**Ditolak:** membatasi lebar section dengan `max-width` (frame terpusat) — bertentangan dengan keputusan "Responsif penuh, bukan frame terpusat" yang sudah dikunci klien.
